const ServiceCard = ({ icon, title, description, price }) => {
  return (
    <div className="bg-misty rounded-lg p-6 shadow-lg hover:shadow-xl transition-all duration-300 flex flex-col h-full relative overflow-hidden group hover:-translate-y-1">
      <div className="absolute inset-0 bg-gradient-to-br from-blue/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
      <div className="absolute -right-4 -top-4 w-24 h-24 bg-blue-light/10 rounded-full blur-xl group-hover:scale-150 transition-transform duration-700"></div>
      
      <div className="text-secondary text-4xl mb-4 relative">
        {icon}
        <span className="absolute -inset-3 bg-secondary/10 rounded-full blur-md opacity-0 group-hover:opacity-100 transition-opacity duration-300"></span>
      </div>
      
      <h3 className="text-2xl font-serif text-secondary/90 mb-3 relative z-10">{title}</h3>
      <p className="text-dark mb-4 flex-grow relative z-10">{description}</p>
      {price && (
        <div className="mt-auto relative z-10">
          <span className="block text-secondary font-medium text-lg">{price}</span>
          <a 
            href="#contact" 
            className="inline-block mt-3 px-4 py-2 bg-secondary/10 text-secondary rounded hover:bg-secondary/20 transition-colors duration-300"
          >
...
const Services = () => {
  ...
  return (
    <div className="container mx-auto relative">
      ...
      <div className="absolute top-1/2 right-1/4 w-40 h-40 bg-secondary/10 rounded-full blur-xl opacity-60"></div>
      <div className="absolute bottom-1/4 left-1/3 w-24 h-24 bg-[#b08bc8]/10 rounded-full blur-xl opacity-40"></div>

      <h2 className="text-4xl font-serif text-center text-secondary mb-4 relative z-10">Mis Servicios</h2>
      <div className="relative mx-auto w-40 h-1.5 bg-gradient-to-r from-transparent via-secondary/60 to-transparent rounded-full mb-6"></div>
      ...
      <div className="mt-16 text-center relative">
        ...
        <h3 className="text-2xl font-serif text-secondary/90 mb-4 relative z-10">¿Necesitas algo especial?</h3>
        ...
        <a 
          href="#contact" 
          className="inline-block px-6 py-3 bg-secondary text-white rounded-md hover:bg-secondary/90 transition-all duration-300 shadow-lg hover:shadow-xl hover:-translate-y-0.5 relative z-10 group"
        >
